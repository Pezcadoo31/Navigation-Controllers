...
class PurePursuitController(Node):
    def __init__(self):
        ...
        self.k_linear = 0.225           # Ganancia para velocidad lineal
        self.lookahead_distance = 0.1   # Distancia de lookahead para Pure Pursuit
    ...
    def calculate_lookahead_point(self):
        ...
        # Si estamos más cerca que la distancia de lookahead, usamos el punto objetivo directamente
        if distance <= self.lookahead_distance:
            return self.target_x, self.target_y
        
        # Calcular un punto a "lookahead_distance" en dirección al objetivo
        ratio = self.lookahead_distance / distance
        lookahead_x = self.odom_x + dx * ratio
        lookahead_y = self.odom_y + dy * ratio
    ...
    def control_loop(self):
        ...
        # Calcular distancia al objetivo real (no al lookahead)
        dx = self.target_x - self.odom_x
        dy = self.target_y - self.odom_y
        distance = math.sqrt(dx**2 + dy**2)
        ...
        # Calcular punto de lookahead
        lookahead_x, lookahead_y = self.calculate_lookahead_point()
        
        # Transformar el punto lookahead al sistema de coordenadas del robot
        dx = lookahead_x - self.odom_x
        dy = lookahead_y - self.odom_y
        
        # Transformar a coordenadas locales )
        local_x = dx * math.cos(self.odom_theta) + dy * math.sin(self.odom_theta)
        local_y = -dx * math.sin(self.odom_theta) + dy * math.cos(self.odom_theta)
        
        # Calcular el ángulo hacia el punto lookahead 
        target_angle = math.atan2(local_y, local_x)
        
        # Calcular curvatura usando la fórmula del Pure Pursuit
        # k = 2 * sin(alpha) / L, donde alpha es el ángulo y L es la distancia de lookahead
        curvature = 2.0 * math.sin(target_angle) / self.lookahead_distance
        
        # Calcular velocidad lineal (reducir a medida que nos acercamos al objetivo)
        linear_speed = min(self.k_linear * distance, self.max_linear_speed)
        
        # Calcular velocidad angular como velocidad lineal * curvatura
